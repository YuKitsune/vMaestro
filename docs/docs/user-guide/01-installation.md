...
3. Copy the `Maestro.json` file into your profile directory (`Documents\vatSys Files\Profiles\<Profile Name>`).
4. Run the `unblock-dlls.bat` helper script (included in the `Maestro.zip` file) to unblock all the `.dll` files.
5. Restart vatSys.
