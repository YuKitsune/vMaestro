...
public class VatsysFixLookup : IFixLookup
{
    public Fix? FindFix(string identifier)
    {
        ...
        if (intersection is not null)
            return new Fix(
                intersection.Name,
                new Maestro.Contracts.Shared.Coordinate(intersection.LatLong.Latitude, intersection.LatLong.Longitude));
        ...
        if (airport is not null)
            return new Fix(
                airport.ICAOName,
                new Maestro.Contracts.Shared.Coordinate(airport.LatLong.Latitude, airport.LatLong.Longitude));
