...
public static class ConfigurationConverter
{
    public static PluginConfigurationV2 ConvertPluginConfiguration(PluginConfiguration v1)
    {
        return new PluginConfigurationV2
        {
            ...
            Airports = v1.Airports.Select(a => ConvertAirport(
                a,
                v1.CoordinationMessages.Templates,
                v1.CoordinationMessages.Templates)).ToArray(),
            Colours = v1.Colours ?? ColourDefaults.Default(),
            Labels = v1.Labels ?? CreateDefaultLabels()
        };
    }
    ...
    public static TrajectoryConfigurationV2[] ConvertArrivalsToTrajectories(ArrivalConfiguration[] arrivals)
    {
        ...
        foreach (var arrival in arrivals)
        {
            ...
            foreach (var (runway, intervalSeconds) in arrival.RunwayIntervals)
            {
        ...
        DepartureAirportConfiguration[] departureAirports)
    {
        ...
        {
            if (airport.FlightTimes.Length == 0)
            {
                // If no flight times configured, create a single entry with default values
                departures.Add(new DepartureConfigurationV2
                {
                    Identifier = airport.Identifier,
                    Aircraft = [new AllAircraftTypesDescriptor()],
                    Distance = airport.Distance,
                    EstimatedFlightTimeMinutes = 0 // TODO: No default flight time available
                });
            }
            else
            {
    ...
    public static ViewConfigurationV2 ConvertView(ViewConfiguration v1)
    {
        ...
        // Convert left ladder (typically feeder fixes)
        if (v1.LeftLadder.Length > 0)
        {
            ladders.Add(new LadderConfigurationV2
            {
                FeederFixes = v1.LeftLadder,
                Runways = []
            });
        }

        // Convert right ladder (typically runways)
        if (v1.RightLadder.Length > 0)
        {
            ladders.Add(new LadderConfigurationV2
            {
                FeederFixes = [],
                Runways = v1.RightLadder
            });
        }
