    ...
    : IRequestHandler<MoveFlightRequest>
{
    public async Task Handle(MoveFlightRequest request, CancellationToken cancellationToken)
    {
        try
        {
            using var exclusiveSequence =
                await sequenceProvider.GetSequence(request.AirportIdentifier, cancellationToken);
            ...
            if (currentSlot is null)
                throw new MaestroException(
                    $"Flight {request.Callsign} not found in sequence for airport {request.AirportIdentifier}.");
            ...
            if (targetSlot is null)
                throw new MaestroException(
                    $"No slot found with ID {request.SlotIdentifier} for runway {request.RunwayIdentifier}.");
            ...
            if (deallocatedFlight is not null)
            {
                if (deallocatedFlight.NoDelay)
                    throw new MaestroException(
                        "Cannot move a flight to a slot that has a no-delay flight already scheduled.");
            ...
            await mediator.Publish(new SequenceChangedNotification(sequence.ToDto()), cancellationToken);
        }
        catch (Exception ex)
        {
            logger.Error(
                ex,
                "Failed to move {Callsign}",
                request.Callsign);
        }
    }
}
