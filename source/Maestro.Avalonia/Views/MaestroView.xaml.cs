...
using AvaloniaShapes = global::Avalonia.Controls.Shapes;
...
public partial class MaestroView : UserControl
{
    ...
    public MaestroView(MaestroViewModel maestroViewModel) : this()
    {
    ...
    void TimerTick(object sender, EventArgs args)
    {
        ...
        if (UtcTimeText != null) UtcTimeText.Text = DateTime.UtcNow.ToString("HH:mm:ss");
    ...
    double MinuteHeight(int timeHorizonMinutes) => LadderCanvas.Bounds.Height / timeHorizonMinutes;

    double MeasureCharacterWidth(string character)
    {
        var textBlock = new TextBlock
        {
            FontFamily = MaestroTheme.FontFamily,
            FontSize = MaestroTheme.FontSize,
            Text = character
        };
    ...
    LayoutInfo CalculateLadderLayout(ViewConfiguration view, LabelLayoutConfiguration labelLayout, double canvasWidth)
    {
        ...
        double horizontalSpacing = 2 * (MaestroTheme.BeveledBorderThickness.Left + MaestroTheme.BeveledBorderThickness.Right);
    ...
    void RedrawSequence(DateTimeOffset referenceTime)
    {
        ...
        var canvasWidth = LadderCanvas.Bounds.Width;
        var canvasHeight = LadderCanvas.Bounds.Height;
    ...
    void DrawFooterBackground(TimelineDirection direction, double canvasWidth)
    {
        ...
        var background = new AvaloniaShapes.Rectangle
        {
            Width = canvasWidth,
            Height = footerHeight,
            Fill = MaestroTheme.BackgroundColor
        };
    ...
    void DrawTimeReferenceBox(TimelineElement timeline, DateTimeOffset referenceTime, TimelineDirection direction)
    {
        ...
        var refTimeText = new TextBlock
        {
            ...
            Foreground = ViewModel.IsScrolling ? MaestroTheme.InteractiveTextColor : MaestroTheme.GenericTextColor
        };
        ...
        var border = new BeveledBorder
        {
            ...
            BorderThickness = MaestroTheme.BeveledBorderThickness,
            Child = refTimeText
        };
    ...
    void DrawLadderInfo(int ladderIndex, ViewConfiguration view, TimelineElement timeline, bool isLeftLadder, TimelineDirection direction, double canvasHeight)
    {
        ...
        if (isLeftLadder)
        {
            ...
            var runwayText = new TextBlock
            {
                ...
                Foreground = MaestroTheme.GenericTextColor
            };
            ...
            if (direction == TimelineDirection.Up)
            {
                runwayText.Loaded += PositionOnCanvas(
                    right: LadderCanvas.Bounds.Width - textX,
                    top: 2);
    ...
    void DrawSlots(DateTimeOffset currentTime, ViewConfiguration view, LayoutInfo layout, double ladderHeight, double ladderYOffset)
    {
        ...
        foreach (var slot in ViewModel.Slots)
        {
            ...
            for (int i = 0; i < view.Ladders.Length; i++)
            {
                ...
                var rectangle = new AvaloniaShapes.Rectangle
                {
                    Width = ticksElement.Width,
                    Height = slotHeight,
                    Fill = MaestroTheme.UnstableColor,
                    Opacity = 1,
                    Cursor = new Cursor(StandardCursorType.Hand)
                };

                rectangle.PointerPressed += (_, args) =>
                {
    ...
    static EventHandler<global::Avalonia.Interactivity.RoutedEventArgs> PositionOnCanvas(
        double? left = null,
        double? top = null,
        double? right = null,
        double? bottom = null,
        ...
        double? y = null)
    {
        return (sender, _) =>
        {
            ...
            if (x.HasValue)
            {
                Canvas.SetLeft(element, x.Value - element.Bounds.Width / 2);
            }

            if (y.HasValue)
            {
                Canvas.SetTop(element, y.Value - element.Bounds.Height / 2);
            }
        };
    }
    ...
    void OnCanvasPointerPressed(object sender, PointerPressedEventArgs e)
    {
        ...
        if (point.Properties.IsLeftButtonPressed)
            OnCanvasLeftClick(sender, e);
        else if (point.Properties.IsRightButtonPressed)
            OnCanvasRightClick(sender, e);
    }

    void OnCanvasLeftClick(object sender, PointerPressedEventArgs e)
    {
    ...
    void OnCanvasRightClick(object sender, PointerPressedEventArgs e)
    {
    ...
    void OnFlightLabelMouseDown(object sender, PointerPressedEventArgs e, FlightDto flight)
    {
        ...
        e.Pointer.Capture(flightLabel);
        e.Handled = true;
    }

    void OnFlightLabelMouseMove(object sender, PointerEventArgs e)
    {
        if (sender is not FlightLabelView flightLabel)
            return;
        ...
        if (_draggingFlightLabel != flightLabel || !e.GetCurrentPoint(null).Properties.IsLeftButtonPressed)
            return;
        ...
        if (!_hasMoved && Math.Abs(deltaY) > 4.0)
        {
            _hasMoved = true;
            _isDragging = true;
    ...
    void OnFlightLabelMouseUp(object sender, PointerReleasedEventArgs e)
    {
        if (sender is not FlightLabelView flightLabel)
            return;
        ...
        if (_hasMoved && _isDragging)
        {
            ...
            var centerY = finalTop + (flightLabel.Bounds.Height / 2);
    ...
    void OnFlightLabelDoubleClick(object sender, TappedEventArgs e, FlightDto flight)
    {
        if (sender is not FlightLabelView flightLabel)
            return;
    ...
    void OnFlightLabelRightClick(object sender, PointerPressedEventArgs e, FlightDto flight)
    {
        if (sender is not FlightLabelView flightLabel)
            return;
    ...
    void SuppressContextMenuIfRequired(object sender, ContextRequestedEventArgs e)
    {
        ...
        foreach (var flight in currentFlights)
        {
            ...
            foreach (var ladderIndex in matchingLadders)
            {
                ...
                if (!_flightLabels.TryGetValue(key, out var flightLabel))
                {
                    ...
                    flightLabel.DoubleTapped += (s, e) => OnFlightLabelDoubleClick(s, e, flight);
                    flightLabel.PointerPressed += (s, e) =>
                    {
                        var point = e.GetCurrentPoint(null);
                        if (point.Properties.IsLeftButtonPressed)
                            OnFlightLabelMouseDown(s, e, flight);
                        else if (point.Properties.IsRightButtonPressed)
                            OnFlightLabelRightClick(s, e, flight);
                    };
                    flightLabel.PointerMoved += OnFlightLabelMouseMove;
                    flightLabel.PointerReleased += OnFlightLabelMouseUp;
                    flightLabel.ContextRequested += SuppressContextMenuIfRequired;
                ...
                Canvas.SetTop(flightLabel, yPosition - flightLabel.Bounds.Height / 2);
