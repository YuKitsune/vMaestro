    ...
    ClockFixture clockFixture)
{
    ...
    public async Task WhenANewFlightIsAdded_ItIsSequenced()
    {
        ...
        var sequence = new SequenceBuilder(airportConfigurationFixture, clockFixture).Build();
        ...
        // Assert
        sequence.Slots.ShouldContain(s => s.Flight != null && s.Flight.Callsign == "QFA123");
    }
    ...
    public async Task WhenFlightHasPassedLandingTime_ItIsMarkedAsLanded()
    {
        ...
        // Advance time to the landing time
        var landingTime = sequence.FindFlight(notification.Callsign)!.ScheduledLandingTime;
        clock.SetTime(landingTime);
    ...
    FlightUpdatedHandler GetHandler(SlotBasedSequence sequence, IClock clock)
    {
